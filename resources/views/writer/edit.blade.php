...
<body>
  ...
  <section class="p-6">
   <h1 class="text-1xl  text-gray-500 font-bold  text-start ml-30">Edit Writer</h1>
    ...
    <form action="{{ route('writer.update', $writer->idwriter) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
    <div class="flex items-center justify-center ml-30">
      <div class="flex-1 p-4">
          ...
          <input type="text" name="username" id="username" value="{{ old('username', $writer->username) }}" 
          ...
          class="w-full border border-gray-300 p-2 mb-4 max-w-md text-gray-500 text-start min-h-[90px]">{{ old('biography', $writer->biography) }}</textarea>
        ...
        <div class="flex-1 flex flex-col items-center justify-center">
          @if($writer->photo)
            <img src="{{ asset('storage/' . $writer->photo) }}" 
