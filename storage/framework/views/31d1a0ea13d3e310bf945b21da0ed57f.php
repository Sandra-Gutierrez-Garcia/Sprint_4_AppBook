  ...
  <section class="p-6">
    ...
    <div class="flex items-center justify-center ml-30">
      <div class="flex-1 p-4">
        ...
        <p class="border border-gray-300 p-2 mb-1 max-w-xl text-gray-500 text-start"><?php echo e($writer->name); ?></p>
        
        <h3 class="text-sm font-medium text-gray-700 mt-4 mb-1">Username</h3>
        <p class="border border-gray-300 p-2 mb-1 max-w-xl text-gray-500 text-start"><?php echo e($writer->username); ?></p>
        
        <h3 class="text-sm font-medium text-gray-700 mt-4 mb-1">Biography</h3>
        <p class="border border-gray-300 p-2 mb-1 text-gray-500 text-start min-h-[90px]"><?php echo e($writer->biography); ?></p>
        
        <div class="mt-4">
          <a href="<?php echo e(route('writer.edit', ['writer' => $writer->idwriter])); ?>" class="hover:bg-orange-300 rounded scale-105 px-4 py-2">Edit petfil</a>
        </div> 
      </div>
      
      <div class="flex-1 ml-30">
        <?php if($writer->photo): ?>
          <img src="<?php echo e(asset('storage/' . $writer->photo)); ?>" alt="Writer Photo" class="w-64 h-64 object-cover rounded mt-2 overflow-hidden rounded-full border-2 border-gray-300 bg-gray-100">
        <?php endif; ?>
      </div>  
    </div>
  </section>
  ...
  <section class="p-6 border-t border-gray-200">
    ...
    <div class="flex gap-4 ml-30">
      ...
      <div class="w-64 rounded-lg flex flex-col justify-center shadow-md shadow-gray-400 bg-white h-62">
        <div class="relative w-full flex justify-center items-center mt-4 mb-2">
          <div class="w-32 h-32 overflow-hidden border-2 border-gray-300 bg-gray-100 flex items-center justify-center">
            <?php if($book->photo): ?>
              <img src="<?php echo e(asset('storage/' . $book->photo)); ?>" alt="book Photo" class="w-32 h-32 object-cover rounded-full">
            <?php endif; ?>
          </div>
        </div>
        
        <p class="text-sm font-medium text-gray-700 mb-1 text-center"><?php echo e($book->title); ?></p>
        
        <div class="flex justify-center border-t border-gray-200">
          <a href="<?php echo e(route('book.edit', ['book' => $book->idbook])); ?>" class="text-sm text-center hover:bg-orange-300 rounded w-16">edit</a>
          <form action="<?php echo e(route('book.destroy', ['book' => $book->idbook])); ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this book?');">
            <?php echo csrf_field(); ?>
      ...
      <div class="flex justify-center">
        <a href="<?php echo e(route('book.create')); ?>" class="mt-20 px-6 text-gray-700 border border-gray-300 rounded hover:bg-orange-300 transition">
          Create Book
        </a>
      </div>
    </div>
...
</body><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/BookApp/AppBook/resources/views/writer/show.blade.php ENDPATH**/ ?>